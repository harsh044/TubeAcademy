...
exports.createCourse = async (req, res) => {
    try {
        // extract data
        let { instructorName, courseName,instructorLogo, courseDescription,totalDuration,thumbnailImage, category, status} = req.body;
        ...
        // validation
        if (!courseName || !instructorLogo || !courseDescription || !category || !thumbnailImage) {
        ...
        const newCourse = await Course.create({instructorName,instructorLogo, courseName, courseDescription, totalDuration,instructor: instructorId, category: categoryDetails._id,
...
exports.updateCourse = async (req, res) => {
    try {
        const { courseId, totalDuration } = req.body; // Get courseId from URL params

        // Validation
        if (!totalDuration) {
            return res.status(400).json({
                success: false,
                message: 'Duration are required'
            });
        }
...
exports.getAllCourses = async (req, res) => {
    try {
        const allCourses = await Course.find({},
            {
                instructorName:true, courseName: true, courseDescription: true, thumbnailImage: true, instructor: true,
