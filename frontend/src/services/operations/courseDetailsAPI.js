...
// ================ create Course Rating  ================
export const checkRating = async (data, token) => {
  const toastId = toast.loading("Loading...")
  let success = false
  try {
    const response = await apiConnector("GET", CHECK_RATING_API, data, {
      Authorization: `Bearer ${token}`,
    })
    console.log("CHECK RATING API RESPONSE............", response)
    if (!response?.data?.success) {
      throw new Error("Could Not Check Rating")
    }
    toast.success("Rating Created Already")
    success = true
  } catch (error) {
    success = false
...
export const handleCertificateView = async (certificateId,token) => {
  try {
    // Make the API call to generate the certificate
    const response = await apiConnector("POST", VIEW_CERTIFICATE_API, { certificateId },{
      Authorization: `Bearer ${token}`, // Token in headers
    });

    console.log("VIEW_CERTIFICATE_API RESPONSE:", response);
...
export const CertificateViewGlobal = async (certificateId) => {
  try {
    // Make the API call to generate the certificate
    const url = `${VIEW_CERTIFICATE_GLOBAL_API}/${certificateId}`;
