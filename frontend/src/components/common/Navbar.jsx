...
import TubeAcademy from "../../assets/Logo/Tube_Academy.png";
    ...
    return (
        <nav className={`z-[10] flex h-14 w-full items-center justify-center border-b-[1px] border-b-richblack-700 text-white translate-y-0 transition-all ${showNavbar}`}>
            <div className='flex w-11/12 max-w-maxContent items-center justify-between '>
                <Link to="/">
                    <img src={TubeAcademy} width={160} height={42} loading='lazy' />
                </Link>
                            ...
                            {link.title === "Catalog" ? (
                                <div
                                    className={`group relative flex cursor-pointer items-center gap-1 ${matchRoute("/catalog/:catalogName")
                                        ? "bg-red-100 text-black rounded-xl p-1 px-3"
                                ...
                                <Link to={link?.path}>
                                    <p className={`${matchRoute(link?.path) ? "bg-red-100 text-black" : "text-richblack-25"} rounded-xl p-1 px-3 `}>
                                        {link.title}
                                    </p>
                                </Link>
                            )}
                ...
                <div className='flex gap-x-4 items-center'>
                    ...
                    {token === null && (
                        <Link to="/login">
                            <button className={` px-[12px] py-[8px] text-richblack-100 rounded-md 
                                 ${matchRoute('/login') ? 'border-[2.5px] border-red-100' : 'border border-richblack-700 bg-richblack-800'} `}>
                                Log in
                            </button>
                        </Link>
                    )}
                    ...
                    {token === null && (
                        <Link to="/signup">
                            <button className={` px-[12px] py-[8px] text-richblack-100 rounded-md 
                                 ${matchRoute('/signup') ? 'border-[2.5px] border-red-100' : 'border border-richblack-700 bg-richblack-800'} `}>
                                Sign Up
                            </button>
                        </Link>
                    )}
