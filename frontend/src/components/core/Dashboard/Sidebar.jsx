...
export default function Sidebar() {
  ...
  return (
    <>
      ...
      <div
        className="sm:hidden text-white absolute left-7 top-3 cursor-pointer z-50"
      ...
      {/* Sidebar Overlay */}
      {openSideMenu && (
        <div className="fixed inset-0 z-40 bg-black bg-opacity-50 sm:hidden" onClick={() => dispatch(setOpenSideMenu(false))}></div>
      )}
  
      {/* Sidebar */}
      {openSideMenu && (
        <div
          className="fixed top-0 left-0 z-50 h-screen w-[250px] flex flex-col border-r-[1px] border-r-richblack-700 bg-richblack-800 py-10 transition-transform transform sm:relative sm:translate-x-0"
          style={{
