            ...
            {open && (
                ...
                >
                    ...
                    >
                        ...
                        {isCatalogOpen && (
                            <div className="absolute left-0 top-full z-10 w-full bg-white rounded-md shadow-lg">
                                {loading ? (
                                    <p className="py-2 px-4 text-sm text-white">Loading...</p>
                                ) : subLinks.length ? (
                                    subLinks.map((subLink, index) => (
                                        <Link
                                            key={index}
                                            to={`/catalog/${subLink.name.split(" ").join("-").toLowerCase()}`}
                                            className="block py-2 px-4 text-sm text-black"
                                        >
                                            {subLink.name}
                                        </Link>
                                    ))
                                ) : (
                                    <p className="py-2 px-4 text-sm text-white">No Categories Found</p>
                                )}
                            </div>
                        )}
                    </div>
